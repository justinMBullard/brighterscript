...
describe('Scope', () => {
    ...
    it('flags assignments with same name as namespace', () => {
        ...
        program.validate();
        (0, testHelpers_spec_1.expectDiagnostics)(program, [
            DiagnosticMessages_1.DiagnosticMessages.variableMayNotHaveSameNameAsNamespace('namea'),
            DiagnosticMessages_1.DiagnosticMessages.variableMayNotHaveSameNameAsNamespace('NAMEA')
        ]);
    });
    ...
    it('allows getting all scopes', () => {
        ...
        (0, chai_1.expect)(scopes.length).to.equal(2);
    });
    describe('addFile', () => {
        it('detects callables from all loaded files', () => {
            const sourceScope = program.getScopeByName('source');
            program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/main.brs`, dest: (0, util_1.standardizePath) `source/main.brs` }, `
                sub Main()
            ...
            `);
            program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/lib.brs`, dest: (0, util_1.standardizePath) `source/lib.brs` }, `
                sub ActionB()
                end sub
            `);
            program.validate();
            (0, chai_1.expect)(sourceScope.getOwnFiles().map(x => x.pathAbsolute).sort()).eql([
                (0, util_1.standardizePath) `${rootDir}/source/lib.brs`,
                (0, util_1.standardizePath) `${rootDir}/source/main.brs`
            ]);
            (0, testHelpers_spec_1.expectZeroDiagnostics)(program);
            (0, chai_1.expect)(sourceScope.getOwnCallables()).is.lengthOf(3);
            (0, chai_1.expect)(sourceScope.getAllCallables()).is.length.greaterThan(3);
        });
        it('picks up new callables', () => {
            ...
            `);
            (0, chai_1.expect)(program.getScopeByName('source').getAllCallables().length).to.equal(originalLength + 2);
        });
    });
    describe('removeFile', () => {
        it('removes callables from list', () => {
            ...
            (0, chai_1.expect)(program.getScopeByName('source').getAllCallables().length).to.equal(initCallableCount - 1);
        });
    });
    describe('validate', () => {
        describe('createObject', () => {
            ...
            it('only adds a single diagnostic when the file is used in multiple scopes', () => {
                ...
                (0, testHelpers_spec_1.expectDiagnostics)(program, [
                    ...
                    DiagnosticMessages_1.DiagnosticMessages.deprecatedBrightScriptComponent('roFontMetrics').code
                ]);
            });
            ...
            it('catches deprecated components', () => {
                ...
                const expectedDiag = DiagnosticMessages_1.DiagnosticMessages.deprecatedBrightScriptComponent('roFontMetrics');
                (0, chai_1.expect)(diagnostics.length).to.eql(1);
                (0, chai_1.expect)(diagnostics[0].code).to.eql(expectedDiag.code);
                (0, chai_1.expect)(diagnostics[0].message).to.contain(expectedDiag.message);
            });
        });
        it('marks the scope as validated after validation has occurred', () => {
            program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/main.bs`, dest: (0, util_1.standardizePath) `source/main.bs` }, `
               sub main()
               end sub
            `);
            let lib = program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/lib.bs`, dest: (0, util_1.standardizePath) `source/lib.bs` }, `
               sub libFunc()
               end sub
            `);
            (0, chai_1.expect)(program.getScopesForFile(lib)[0].isValidated).to.be.false;
            program.validate();
            (0, chai_1.expect)(program.getScopesForFile(lib)[0].isValidated).to.be.true;
            lib = program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/lib.bs`, dest: (0, util_1.standardizePath) `source/lib.bs` }, `
                sub libFunc()
                end sub
            `);
        ...
        it('detects calls to unknown callables', () => {
            ...
            program.validate();
            (0, testHelpers_spec_1.expectDiagnostics)(program, [
                DiagnosticMessages_1.DiagnosticMessages.callToUnknownFunction('DoB', 'source')
            ]);
        });
        it('recognizes known callables', () => {
            ...
            program.validate();
            (0, testHelpers_spec_1.expectDiagnostics)(program, [
                DiagnosticMessages_1.DiagnosticMessages.callToUnknownFunction('DoC', 'source')
            ]);
        });
        ...
        it('Emits validation events', () => {
            ...
            program.plugins = new PluginInterface_1.default([], new Logger_1.Logger());
            ...
            (0, chai_1.expect)(plugin.beforeScopeValidate.callCount).to.equal(2);
            (0, chai_1.expect)(plugin.beforeScopeValidate.calledWith(sourceScope)).to.be.true;
            (0, chai_1.expect)(plugin.beforeScopeValidate.calledWith(compScope)).to.be.true;
            (0, chai_1.expect)(plugin.onScopeValidate.callCount).to.equal(2);
            (0, chai_1.expect)(plugin.onScopeValidate.getCalls().map(x => x.args[0].scope.name).sort()).to.eql(scopeNames);
            (0, chai_1.expect)(plugin.afterScopeValidate.callCount).to.equal(2);
            (0, chai_1.expect)(plugin.afterScopeValidate.calledWith(sourceScope)).to.be.true;
            (0, chai_1.expect)(plugin.afterScopeValidate.calledWith(compScope)).to.be.true;
        });
        describe('custom types', () => {
            ...
            it('finds custom types from other other files', () => {
                ...
                `);
                program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/MyClass.bs`, dest: (0, util_1.standardizePath) `source/MyClass.bs` }, `
                    class MyClass
                    end class
                `);
                program.validate();
            ...
            it('finds custom types from other other files', () => {
                ...
                `);
                program.setFile({ src: (0, util_1.standardizePath) `${rootDir}/source/MyNameSpace.bs`, dest: (0, util_1.standardizePath) `source/MyNameSpace.bs` }, `
                    namespace MyNameSpace
                      class MyClass
                      end class
                    end namespace
    ...
    describe('inheritance', () => {
        it('inherits callables from parent', () => {
            ...
            (0, chai_1.expect)(childScope.getAllCallables().map(x => x.callable.name)).not.to.include('parentSub');
            program.setFile('components/parent.xml', (0, testHelpers_spec_1.trim) `
                <?xml version="1.0" encoding="utf-8" ?>
                <component name="parent" extends="Scene">
            ...
            `);
            program.validate();
            (0, chai_1.expect)(childScope.getAllCallables().map(x => x.callable.name)).to.include('parentSub');
        });
    });
    describe('detachParent', () => {
        it('does not attach global to itself', () => {
            (0, chai_1.expect)(program.globalScope.getParentScope()).not.to.exist;
        });
    });
    describe('getDefinition', () => {
        it('returns empty list when there are no files', () => {
            ...
            (0, chai_1.expect)(scope.getDefinition(file, vscode_languageserver_1.Position.create(0, 0))).to.be.lengthOf(0);
        });
    });
    describe('getCallablesAsCompletions', () => {
        it('returns documentation when possible', () => {
            ...
            (0, chai_1.expect)(completions).to.be.length.greaterThan(0);
            //it should find documentation for completions
            (0, chai_1.expect)(completions.filter(x => !!x.documentation)).to.have.length.greaterThan(0);
        });
    });
    ...
    describe('buildEnumLookup', () => {
        it('builds enum lookup', () => {
            ...
            `);
            // program.validate();
            (0, chai_1.expect)([...sourceScope.getEnumMap().keys()]).to.eql([
                'foo',
                'test.foo2',
                'foo3'
            ]);
